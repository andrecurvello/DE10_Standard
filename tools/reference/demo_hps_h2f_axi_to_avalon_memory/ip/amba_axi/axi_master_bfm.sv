...
module axi_frontend (
   ...
   // DODGY VARIABLES, THAT'D BE THE ENTRY POINT OF ALL OF THIS.----------------
   logic                      pgrant;   
...
// ---------------------------------------------------------------
    ...
    //AXI write address bus ------------------------------------------
    output   [AXI_ID_W-1:0]      axs_awid;
    output   [AXI_ADDRESS_W-1:0] axs_awaddr;
    output   [ 3:0]              axs_awlen;   //burst length is 1 + (0 - 15)
    output   [ 2:0]              axs_awsize;  //size of each transfer in burst
    output   [ 1:0]              axs_awburst; //for bursts>1, accept only incr burst=01
    output   [ 1:0]              axs_awlock;  //only normal access supported axs_awlock=00
    output   [ 3:0]              axs_awcache; 
    output   [ 2:0]              axs_awprot;
    output                       axs_awvalid; //master addr valid
    input                        axs_awready; //slave ready to accept
 
    //AXI write data bus ---------------------------------------------
    output   [AXI_ID_W-1:0]      axs_wid;
    output   [AXI_DATA_W-1:0]    axs_wdata;
    output   [AXI_NUMBYTES-1:0]  axs_wstrb;  //1 strobe per byte
    output                       axs_wlast;  //last transfer in burst
    output                       axs_wvalid; //master data valid
    input                        axs_wready;   //slave ready to accept

    //AXI write response bus -----------------------------------------
    input  [AXI_ID_W-1:0]      axs_bid;
    input  [ 1:0]              axs_bresp;
    input                      axs_bvalid;
    output                     axs_bready;
   
    //AXI read address bus -------------------------------------------
    output   [AXI_ID_W-1:0]      axs_arid;
    output   [AXI_ADDRESS_W-1:0] axs_araddr;
    output   [ 3:0]              axs_arlen;   //burst length - 1 to 16
    output   [ 2:0]              axs_arsize;  //size of each transfer in burst
    output   [ 1:0]              axs_arburst; //for bursts>1, accept only incr burst=01
    output   [ 1:0]              axs_arlock;  //only normal access supported axs_awlock=00
    output   [ 3:0]              axs_arcache; 
    output   [ 2:0]              axs_arprot;
    output                       axs_arvalid; //master addr valid
    input                        axs_arready; //slave ready to accept
 
    //AXI read data bus ----------------------------------------------
    input  [AXI_ID_W-1:0]      axs_rid;
    input  [AXI_DATA_W-1:0]    axs_rdata;
    input  [ 1:0]              axs_rresp;
    input                      axs_rlast; //last transfer in burst
    input                      axs_rvalid;//slave data valid
    output                     axs_rready;//master ready to accept
...
** -------------------------------------------------------------------------- */
    ...
    typedef logic [LeftIndex(AXI_BURSTCOUNT_W):0                          ]  AXIBurstCount_t;
    typedef logic [LeftIndex(AXI_TRANSACTIONID_W):0                       ]  AXITransactionId_t;
    typedef logic [LeftIndex(MAX_BURST_SIZE):0][LeftIndex(AXI_DATA_W):0   ]  AvalonData_t;
    typedef logic [LeftIndex(MAX_BURST_SIZE):0][LeftIndex(AXI_NUMBYTES):0 ]  AvalonWStrobe_t;
